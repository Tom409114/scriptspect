...
| 候选提交的最终 CI | `HOSTED PENDING` | 本地检查通过；修复分支尚无对应的 GitHub PR/三 OS × Node 22/24 完整 run。 |
...
- Vitest：53 个 test files 通过，564 passed、6 skipped；skips 不被计作通过的 hosted/symlink 证据。
- parser/rules 定向 coverage：statements 96.46%、branches 91.61%、functions 97.88%、lines 98.43%，均高于 90% 本地门槛。
- TypeScript typecheck：通过。
- Biome `--error-on-warnings`：158 个受支持的源码/配置文件通过；Biome 不处理本报告的 Markdown 文件。
- release/workflow/benchmark 定向回归：4 个 test files、63 tests 通过。
- Action 定向回归：16 tests 通过；bundle 在无 `node_modules` 的隔离消费场景中可运行。
- `actionlint 1.7.12 -shellcheck=`：无诊断，exit 0；此命令明确关闭了 shellcheck。

本地 actionlint 已证明 workflow YAML/expression 契约，但完整 shellcheck 没有形成可接受的成功证据，因此本报告不把 shell 检查标为通过；应在 Linux hosted validation 中补齐。最终候选提交产生后，上述本地数字也必须重新执行，不能沿用为新 commit 的证明。
...
| DOD-03 | `LOCAL PASS` | 新增 PS051 具备 registry metadata、target-aware 正负/边界 fixtures 与[生成规则文档](../rules/PS051.md)；registry/docs consistency 由测试覆盖。 | 最终 PR 运行一致性与生成物检查。 |
...
| DOD-09 | `PR PENDING` | 设计已记录 scope、语义风险、测试与 rollback；变更按多个可审阅 commits 拆分。 | 推送修复分支并创建新的审阅 PR；填写实际 risk/tests/semantics/rollback，全部 checks 通过后再合并。旧 PR #62 不应按旧内容直接合并。 |
...
6. **CI 与 release code**：PR jobs 最小权限、frozen lock、真实 Action consumer、package consumer、coverage/benchmark gates；release-please 只开 PR，coordinator 构建并持久化唯一候选，exact-tag publisher 使用 OIDC，state/aliases 使用序列化和 compare-and-update 恢复语义。
...
1. 推送最终分支并创建新的修复 PR；不要合并旧 PR #62 来替代这批修复。
...
2. 为 immutable `v*.*.*` 与 floating `v0`/`v0.*` 建立不同 tag policies；记录 coordinator actor、ruleset IDs 与 non-production permission drill。
...
- `v0.1.0` immutable tag、公开 GitHub Release、authoritative tarball、`SHA256SUMS`、release manifests 与 final verification record。
