...
- 本次最终核签所验证的实现 HEAD：`feat/v0.1-hardening` at `8034e1f48062172c0d42b05121f46a87a674c109`
...
- Vitest：55 个 test files 通过，597 passed、6 skipped；skips 不被计作通过的 hosted/symlink 证据。
...
- release/workflow 定向回归：4 个 test files、80 tests 通过。
...
本地 actionlint 与 workflow contract tests 已证明除上述新字段外的 YAML/expression 契约；GitHub hosted parser 仍需证明 `queue: max` 的线上接受情况。完整 shellcheck 没有形成可接受的成功证据，因此本报告不把 shell 检查标为通过；应在 Linux hosted validation 中补齐。最终候选提交产生后，上述本地数字也必须重新执行，不能沿用为新 commit 的证明。
